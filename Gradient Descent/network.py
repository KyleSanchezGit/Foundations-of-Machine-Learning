...
class Network(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    softmax_outputs = np.exp(shifted_scores)
    softmax_outputs /= np.sum(softmax_outputs, axis=1, keepdims=True)

    correct_logprobs = -np.log(softmax_outputs[range(N), y])
